...
CASE 
 ...
 AS `rating` ')
    ...
    public function getDocTimeSlotForhospital($hospitalId, $doctorUserId, $avia = NULL) {
        ...
        $this->db->select('doctorAvailability_id id, doctorAvailability_day day')
                ->from('qyura_doctorAvailability')
                ->join('qyura_doctorAvailabilitySession', 'qyura_doctorAvailability.doctorAvailability_id = qyura_doctorAvailabilitySession.doctorAvailability_refferalId AND doctorAvailability_refferalId = '.$hospitalId.' ' , 'left')
                ->order_by('day', 'ASC');
    ...
    public function getDoctorSessionForHospital($id, $hospitalId,$slotId = NULL) {
       // echo $avia; exit;
        $where =array('hospital_usersId' => $hospitalId,'doctorAvailability_doctorAvailabilityId'=>$id);
        if($slotId != NULL){
            $where["doctorAvailabilitySession_id"] = $slotId;
        }
